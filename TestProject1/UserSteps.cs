...
{
    [Binding]
    public class UserSteps(IWebDriver webDriver)
    {
        ...
        public void GivenTheUserIsOnTheHomepage()
        {
            ...
            var (baseUrl, _, _, _) = TestDataFactory.NavigationTestData();
            driver.Navigate().GoToUrl(baseUrl);
            Log.Information("User navigated to the homepage: {Url}", baseUrl);
        }

        [When(@"the user tap on ""(.*)""")]
        public void WhenTheUserTapOn(string word)
        {
            Log.Debug("{Word} page opening.", word);
            var aboutPage = new AboutPage(driver);
            word = aboutPage.GetAboutHeader();
            Log.Information("{Word} page opened.", word);

        }
        [Then(@"the URL should be  ""(.*)""")]
        public void VerifyingURL()
        {
            var (_, _, expectedUrl, _) = TestDataFactory.NavigationTestData();
            Log.Debug("{ExpectedURL} verifying.", expectedUrl);
            driver.Url.Should().Be(expectedUrl);
            Log.Information("{ExpectedURL} verified", expectedUrl);
        ...
        public void ThenThePageTitleShouldBe(string expectedTitle)
        {
            ...
            driver.FindElement(By.TagName("h1")).Text.Should().Be(expectedTitle);
